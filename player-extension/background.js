// CyberPlayer background service worker.
// Sniffs network traffic for media responses and tracks the *real* videos on a
// page (filtering out ads and stream fragments) so the popup can play them
// in-extension. State lives in chrome.storage.session so it survives
// service-worker restarts. Unlike a downloader, this worker never touches
// chrome.downloads — detection is all it does.
...
const STREAM_EXT = ["m3u8", "mpd"]; // manifests, played via hls.js / native
const MAX_ITEMS_PER_TAB = 150;

// HLS/DASH fragments are useless individually and one stream fires hundreds of
// them — never list these. The manifest is the playable thing.
...
function classify(url, contentType) {
  ...
  // Stream fragments are never listed — only their manifest is playable.
...
// ── Messaging from the popup / player page ───────────────────────────────────
